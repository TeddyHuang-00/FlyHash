...
Welcome to `FlyHash` contributor's guide.
...
If you experience bugs or general issues with `FlyHash`, please have a look
...
You can help improve `FlyHash` docs by making them more readable and coherent, or
by adding missing information and correcting mistakes.

`FlyHash` documentation uses [Sphinx] as its main documentation compiler.
This means that the docs are kept in the same repository as the project code, and
that any documentation update is done in the same way was a code contribution.

:::{tip}
Please notice that the [GitHub web interface] provides a quick way of
propose changes in `FlyHash`'s files. While this mechanism can
...
The `FlyHash` class should be the only entry point for the users of the package.

The user should be able to create a `FlyHash` object and use it to
compute the hash of a fixed-length numeric array.
...
```
conda create -n FlyHash python=3 six virtualenv pytest pytest-cov
conda activate FlyHash
```
...
3. Clone this copy to your local disk:
   ...
   ```
   git clone git@github.com:YourLogin/FlyHash.git
   cd FlyHash
   ```
...
5. Install [pre-commit]:
   ...
   `FlyHash` comes with a lot of hooks configured to automatically help the
...
[repository]: https://github.com/TeddyHuang-00/FlyHash
[issue tracker]: https://github.com/TeddyHuang-00/FlyHash/issues
